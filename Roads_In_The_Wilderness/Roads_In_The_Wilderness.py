...
outp = open("out.txt", "w")
weights = {}    # dict of edges and their weight
weights['o'] = 1
weights['w'] = 2
weights['s'] = 4
weights['m'] = 6
        ...
        for i in range (lastx-1, lastx+2):
            for j in range(lasty-1, lasty+2):
                ...
                if lastx % 2 == 0 and ((i == lastx-1 and j == lasty+1) or (i == lastx+1 and j == lasty+1)):
                    ...
                    continue
                G.add_edge(tupToNode((lastx, lasty)), tupToNode((i, j)), weight = weights[mmap[i][j]])
...
for c in cities.keys():
    ...
    for r in cities[(x, y)]:
        if r == "s":
            G.add_edge(Rs, node, weight = weights[mmap[x][y]])
            res_dict[Rs] = "s"
        elif r == "o":
            G.add_edge(Ro, node, weight = weights[mmap[x][y]])
            res_dict[Ro] = "o"
        elif r == "t":
            G.add_edge(Rt, node, weight = weights[mmap[x][y]])
            res_dict[Rt] = "t"
        elif r == "w":
            G.add_edge(Rw, node, weight = weights[mmap[x][y]])
            res_dict[Rw] = "w"
        elif r == "p":
            G.add_edge(Rp, node, weight = weights[mmap[x][y]])
            res_dict[Rp] = "p"
        else:
            G.add_edge(Rc, node, weight = weights[mmap[x][y]])
            res_dict[Rc] = "c"
...
res = [list(c) for c in res]    # each element in the list is a list of tuples containing resources. ex: index 0 has a list of tuples size 2, 1 has of size 3 etc...
for l in res:
    ...
    for rgroup in res:
        for r in rgroup:    # ex: r = ('w', 'o', 's')
            ...
            if set(lr).issubset(set(cities[c])):
                G.add_edge(Rcomb[r], tupToNode(c), weight = weights[mmap[c[0]][c[1]]])
        group_len += 1
...
for r in Rall:
    ...
    for c in cities.keys():
        ...
        path_dist = dist[node] - weights[mmap[c[0]][c[1]]]
        while curr != r:
    ...
    for r in rg:
        ...
        for c in cities.keys():
            ...
            if len(pred[node]) == 0:    # city has resource
                city_path[c][r] = (0, [c])
                continue
            path_cost = dist[node] - weights[mmap[c[0]][c[1]]]
            curr = pred[node][0]    # multi choice
            curr_tup = nodeToTup(curr)  #(int(curr/N), (curr%N))
            while curr != Rcomb[r]:
